...
def dimensionality_reduction(signal, times, response_window, max_var=99, min_snr=1.1,
    ...
    if not n_components:
        n_components = signal.shape[0] # WS - number of sources/ROIs

    Vk, eigenvalues = get_svd(signal, times, response_window, n_components)
    var_exp = 100 * eigenvalues**2/np.sum(eigenvalues**2)

    signal_svd = apply_svd(signal, Vk)  # WS -  

    max_dim = calc_maxdim(eigenvalues, max_var)

    signal_svd = signal_svd[:max_dim, :]  # WS - only take the PC's that are necessary for the max_var
    ...
    signal_svd = signal_svd[snrs > min_snr, :] # WS - by default min_snr is set to 1.1 (signal to noise ratio)
    snrs = snrs[snrs > min_snr]

    Nc = signal_svd.shape[0] # WS - which is the number of PC's that survived dimensionality reduction
...
def state_transition_quantification(signal, times, k, baseline_window, response_window, embed=False,
    ...
    n_dims = signal.shape[0] # WS - the number of principal components
    if n_dims == 0:
    ...
    if n_response <= 1 or n_baseline <= 1:  # WS - the pre or post perturbational timeseries has only one timepoint
        print('Warning: Bad time interval defined.')

    baseline = signal[:, :, base_ini_ix:base_end_ix]  # WS - signal is not 2D, check line 207
    ...
    for i in range(n_dims):
        ...
        thresholds[:, i] = np.linspace(min_thr, max_thr, n_steps) # WS - n_steps by default is 100, so 100 different thresholds are tested
...
def recurrence_matrix(signal, mode, thr=None):
    ...
    for i in range(n_dims):
        ...
        D = D - D.T  # WS - calculates the difference between the values of all time points, yielding a symmetrical matrix
        R[i, :, :] = D
    R = np.linalg.norm(R, ord=2, axis=0) # WS - axis to get the norm for each ith matrix
...
def calc_maxdim(eigenvalues, max_var):
    ...
    var = eigenvalues ** 2 # WS - WHY??
    var_p = 100 * var/np.sum(var)
    var_cum = np.cumsum(var_p)
    max_dim = len(eigenvalues) - np.sum(var_cum >= max_var) + 1 # WS - sum the PC's that you dont need, and 
                                                                # extract them from the number of eigenvalues
                                                                # max_dim then contains the nr of PC's needd
                                                                # to account for the max_var (e.g. 99%)
    return max_dim
...
def preprocess_signal(signal_evk, times, time_window, baseline_corr=False, resample=None,
    ...
    if baseline_corr:
        signal_evk = baseline_correct(signal_evk, times, delta=-50) # WS - should this -50 value be hard coded??
...
def baseline_correct(Y, times, delta=0):
    ''' Baseline correct signal using times < delta ''' # WS - puts mean to zero basically
